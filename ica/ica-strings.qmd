...
classes |>
  mutate(sping = str_detect(sem, "SP"))
...
```{r}
#you can't w str sub because not every name has the same amount of characters
```
...
**there's nothing in between to seperate them by**
...
lala <- courses_clean |>
  filter(dept == "MUSI") |>
  group_by(instructor) |>
  summarize(totalstudents = sum(enroll)) |>
...
```{r}
enrollments |>
  filter(instructor == "Xin Yang") |>
  distinct(days, time, instructor, .keep_all = TRUE) |>
  ggplot(aes(x = time, y = enroll)) +
  geom_point()
