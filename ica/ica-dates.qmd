...
penguins|>
  ...
  mutate(cm_bill_length = bill_length_mm/10)|>
  select(species, cm_bill_length)|>
  arrange(desc(cm_bill_length))
...
*Getting rid of that much data affects your entire chart, sometimes getting rid of things you didn't want to*
...
"one shows today's month, the other shows the order of all months"
```

```{r}
# What does this number mean?
week(today)
"what week today is in the scope of the entire year"
```
...
"one gives the day in the scope of the month, the other does in the scope of a year"
```
...
"gives today's day of the week, the other gives the order of all days in the week"
```
...
today < ymd("2024-02-14")
"the first: today's day is after february 14th, the second: today's date is before february 14th
```
...
```{r}
ggplot(daily_births, aes(x = day, y = total_births)) +
  geom_point() +
  geom_line()
```
...
Birthdays |>
  ...
  summarize(total_births = sum(births)) |>
  ggplot(aes(x = month_name, y = total_births)) + 
    geom_boxplot() 

"The largest amount of total births happen from july to september"
